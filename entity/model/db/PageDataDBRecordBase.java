...
public class PageDataDBRecordBase extends ServletRecord
{
   ...
   public static final int URLPREFIX = 4;
   public static final int GOOGLEANALYTICS = 5;
   public static final int CSSNAME = 6;
   ...
   public     void setCallBack( String aCallBack ) { callBack = aCallBack; }
...
//    log.logEvent( log.TRACE, "PageDataDBRecordBase basic constructor");
   ...
   protected void createColumnObjects( DBData aDbData )
   {
      ...
      dbColumns[4] = new ServletStringColumn( aDbData, "PAGEDATA.URLPREFIX", "PAGEDATA", "urlPrefix" );
      if ( urlPrefix == null ) urlPrefix = ((ServletStringColumn)dbColumns[4]).getValue(); // if it hasn't already been initialized...
      dbColumns[5] = new ServletStringColumn( aDbData, "PAGEDATA.GOOGLEANALYTICS", "PAGEDATA", "googleAnalytics" );
      if ( googleAnalytics == null ) googleAnalytics = ((ServletStringColumn)dbColumns[5]).getValue(); // if it hasn't already been initialized...
      dbColumns[6] = new ServletStringColumn( aDbData, "PAGEDATA.CSSNAME", "PAGEDATA", "cssName" );
      if ( cssName == null ) cssName = ((ServletStringColumn)dbColumns[6]).getValue(); // if it hasn't already been initialized...
      dbColumns[7] = new ServletStringColumn( aDbData, "PAGEDATA.CALLBACK", "PAGEDATA", "callBackParms" );
      if ( callBack == null ) callBack = ((ServletStringColumn)dbColumns[7]).getValue(); // if it hasn't already been initialized...
   }
   ...
   protected void refreshLocalVariables()
   {
      ...
      urlPrefix = ((ServletStringColumn)dbColumns[4]).getValue();
      googleAnalytics = ((ServletStringColumn)dbColumns[5]).getValue();
      cssName = ((ServletStringColumn)dbColumns[6]).getValue();
      callBack = ((ServletStringColumn)dbColumns[7]).getValue();
   }
   ...
   protected void refreshColumnObjects()
   {
      ...
      ((ServletStringColumn)dbColumns[4]).setValue( urlPrefix );
      ((ServletStringColumn)dbColumns[5]).setValue( googleAnalytics );
      ((ServletStringColumn)dbColumns[6]).setValue( cssName );
      ((ServletStringColumn)dbColumns[7]).setValue( callBack );
   }
   ...
   public String toString( String prefix )
   {
      ...
      try 
      {
         ...
         retString += "\n"+ prefix +   "    callBack (PAGEDATA.CALLBACK) -- ";
         retString += "localVar=\"" + callBack.toString() + "\";  ";
   ...
   public String toCSVHeaderString(PageDataViewBase viewDef, int ndx )
   {
      ...
      try 
      {
         ...
         if( viewDef.showCallBack ) retString += commaSeparator(retString) + "callBack" + ndxSuffix;
      }
      catch (Exception e)
      {
   ...
   public String toCSVString(PageDataViewBase viewDef )
   {
      ...
      try 
      {
         ...
         if( viewDef.showCallBack ) retString += commaSeparator(retString) + CSVCell( callBack);
      }
      catch (Exception e)
      {
         retString += e.toString();
   ...
   public void getSQLCreateStatement()
   {
      ...
      sysLogger.info( "CREATE TABLE PAGEDATA ( PAGEDATA.PAGEDESCRIPTION char, PAGEDATA.KEYWORDS char, PAGEDATA.GOOGLEVERIFY char, PAGEDATA.BINGVERIFY char, PAGEDATA.URLPREFIX char, PAGEDATA.GOOGLEANALYTICS char, PAGEDATA.CSSNAME char, PAGEDATA.CALLBACK char );" );
   }
   ...
   public static DBQuery getDBQuery( String aTableName, DBData aDbData, HttpServletRequest request, String label, int logic )
   {
      ...
      buildFieldQuery( aTableName, dBQuery, request, "callBackParms", label ); 
      return( dBQuery );
   }
